...
public:
	//--------------------------------------自定义的函数-------------------------------------------------------
	static DWORD ReadDWORD(DWORD pid, VMMDLL_MAP_MODULEENTRY module, DWORD pCur) //读内存整数型
	{
		DWORD value = 0;
		VMMDLL_MemRead(pid, pCur, reinterpret_cast<PBYTE>(&value), sizeof(DWORD));
		return value;
	}

	static void* ReadPointer(DWORD pid, VMMDLL_MAP_MODULEENTRY module, DWORD pCur) // 读内存指针型
	{
		void* pointer = nullptr;
		VMMDLL_MemRead(pid, pCur, reinterpret_cast<PBYTE>(&pointer), sizeof(void*));
		return pointer;
	}

	float ReadFloat(DWORD pid, VMMDLL_MAP_MODULEENTRY module, DWORD pCur) //读单浮点小数型
	{
		float value;

		VMMDLL_MemRead(pid, pCur, reinterpret_cast<PBYTE>(&value), sizeof(float));

		return value;
	}

	static double ReadDouble(DWORD pid, VMMDLL_MAP_MODULEENTRY module, DWORD pCur)//读双精度小数型
	{
		double value;

		VMMDLL_MemRead(pid, pCur, reinterpret_cast<PBYTE>(&value), sizeof(double));
